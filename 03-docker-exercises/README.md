...
Then, at http://localhost:8081 I can log in with app:passwd123!. The team members where successfully added to the members db:
...
You have 2 containers for your app, and you don't want to start them separately all the time. So you configure a docker-compose file for both:
...
### Solution 5
> **Note:** A droplet that was already created during the lecture was used here. To access it via the browser the firewall as to be adjusted to allow inbound connections on port 8081.
...
1. Docker prevents http connections (without ssl certificate) by default. Adapt the daemon.json to let the docker cli connect to my http server:
   ...
   ```
2. Now I can log in to the repo with `docker login`, which requires me to enter the credentials of the Nexus3 user which was just created in the Nexus UI:
   ```cmd
